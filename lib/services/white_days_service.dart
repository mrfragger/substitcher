...
class WhiteDaysService {
  ...
  static Future<WhiteDays?> _fetchWhiteDays() async {
    try {
      ...
      if (whiteDays.isEmpty) return null;

      // 1st of the current Hijri month = day 13's Gregorian date minus 12 days
      final day13Date = DateTime.parse(whiteDays[0].formattedDate);
      final monthStart = day13Date.subtract(const Duration(days: 12));

      // Fetch day 29 (always exists) to derive next month's possible start dates
      DateTime day29Date;
      final day29Url = 'http://api.aladhan.com/v1/hToG?date=29-$targetMonth-$targetYear';
      final day29Response = await http.get(Uri.parse(day29Url));

      if (day29Response.statusCode == 200) {
        final day29Data = jsonDecode(day29Response.body);
        final gregorianInfo = day29Data['data']['gregorian'];
        final originalDate = gregorianInfo['date'] as String;
        final parts = originalDate.split('-');
        day29Date = DateTime(int.parse(parts[2]), int.parse(parts[1]), int.parse(parts[0]));
      } else {
        // Fallback estimate if the call fails
        day29Date = day13Date.add(const Duration(days: 16));
      }

      final nextMonthNum = targetMonth < 12 ? targetMonth + 1 : 1;
      final nextMonthYr = targetMonth < 12 ? targetYear : targetYear + 1;
      final nextMonthName = _monthNames[nextMonthNum - 1];
      ...
      final result = WhiteDays(
        ...
        year: targetYear,
        days: whiteDays,
        monthStartDate: fmt(monthStart),
        nextMonthName: nextMonthName,
        nextMonthYear: nextMonthYr,
        nextMonthStartCandidates: [
          fmt(day29Date.add(const Duration(days: 1))),
          fmt(day29Date.add(const Duration(days: 2))),
        ],
      );
